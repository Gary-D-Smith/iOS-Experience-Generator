...
export function parseOutputs(): IOSOutput[] {
  const filePath = path.join(process.cwd(), 'outputs.csv');
...
export function getRandomOutput(): { category: string; item: string; library: string; link: string } {
  const filePath = path.join(process.cwd(), 'outputs.csv');
  const content = fs.readFileSync(filePath, 'utf-8');
  const rows = parseCSV(content);
